...
完整计算 20 s、共 2000 个耦合窗，**全部收敛**（`unconverged_windows = 0`），
平均每窗 2.06 次耦合迭代、最多 4 次，说明 IQN-ILS 加速收敛良好。取末 2 s
的稳态平均值与 Hron & Turek 2006 参考值比较：
...
末 2 s 内位移峰峰值约 1e-9 m、力峰峰值约 2e-4 N/m，确认已达稳态。位移与力的
误差均在约 3% 以内，其中力误差小于 1%。力通过 OpenFOAM `forces` 函数对象对
「圆柱 + 梁」表面总力（压力 + 黏性）积分得到，再除以 0.01 m 厚度换算为 N/m。

## 8. 结论与局限

- 本复现证明 OpenFOAM + CalculiX + preCICE 的分区耦合链路可稳定运行并给出与
  文献参考值在约 3% 以内一致的结果，完整走通「建模 → 耦合求解 → 后处理 →
  参考值比对」闭环。
